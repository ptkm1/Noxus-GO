...
type DashCard = { to: string; title: string; description: string; icon: LucideIcon };

const adminCards: DashCard[] = [
  { to: "/tabelas-preco", title: "Tabelas de preço", description: "Tabelas e preços por produto", icon: Table },
  { to: "/produtos", title: "Produtos", description: "Lista, edição e base de cadastro", icon: Package },
  { to: "/vendedores", title: "Vendedores", description: "Comissão, gestor e produtos liberados", icon: Users },
  { to: "/comissao", title: "Comissões e metas", description: "Faixas progressivas e metas mensais", icon: Target },
  { to: "/clientes", title: "Clientes", description: "Cadastro e vínculo com vendedor", icon: UserCircle },
...
const managerCards: DashCard[] = [
  { to: "/rastreio", title: "Rastreio ao vivo", description: "Acompanhe a sua equipe no mapa", icon: Navigation },
  { to: "/visitas", title: "Visitas em campo", description: "Check-ins dos vendedores", icon: MapPin },
  { to: "/vendas", title: "Vendas", description: "Pedidos da equipe (leitura)", icon: ShoppingCart },
];
            ...
            <Link key={c.to} to={c.to} className="block card-hover">
              <article className="surface-card flex h-full gap-4 p-4 transition-colors hover:border-primary/30">
                ...
                <div className="min-w-0">
                  ...
                  <p className={cn("mt-1 text-sm text-muted-foreground")}>{c.description}</p>
                </div>
              </article>
            </Link>
          );
