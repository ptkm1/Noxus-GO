...
export function ReportOrdersPage() {
  ...
  async function generate() {
    ...
    try {
      ...
      await downloadPdf(
        `/admin/reports/orders.pdf?${q.toString()}`,
        romaneio ? "relatorio-pedidos-romaneio.pdf" : "relatorio-pedidos.pdf",
      );
    } catch (e) {
  ...
  return (
    ...
    >
      ...
      <ReportField label="Romaneio">
        <label className="flex items-center gap-2 text-sm">
          <Checkbox
            checked={romaneio}
            onCheckedChange={(v) => setRomaneio(v === true)}
          />
