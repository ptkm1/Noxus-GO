/**
 * AC Raiz ICP-Brasil v10 (ITI).
 * Fonte: https://acraiz.icpbrasil.gov.br/credenciadas/RAIZ/ICP-Brasilv10.crt
 * Válida até 2032-07-01. Os webservices da SEFAZ (SP, SVRS, etc.) usam
 * SSL emitido sob esta raiz, que não está no bundle Mozilla do Node.js.
 */
export const ICP_BRASIL_V10_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;
